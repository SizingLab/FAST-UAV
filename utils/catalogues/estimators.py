...
class NearestNeighbor:
    ...
    def predict2(self, X):
        ...
        # select upper values of parameters in database if asked by user  # TODO: solve for multiple features criteria
        for i, x_name in enumerate(X_names):
            if crits[i] == "next":
                for j in list(indices[0]):
                    neigh = df.loc[j]
                    if neigh[x_name] >= X[i][0]:
                        return df.iloc[[j]]
        ...
        df_y = df.iloc[[indices[0][0]]]  # get closest neighbor
        return df_y
