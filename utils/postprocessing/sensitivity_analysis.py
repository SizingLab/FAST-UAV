...
def doe_salib(method_name: str, x_dict: dict, y_list: List[str], conf_file: str, ns: int,
    ...
    :param method_name: Sobol with Monte Carlo simulations or Morris
    ...
    if 'objective' in prob_definition.keys():
        ...
        prob = om.Problem()
        prob.model.add_subsystem('sub_prob', SubProbComp(conf=conf,
                                                         x_list=x_list,
                                                         y_list=y_list,
                                                         ),
                                 promotes=['*'])
    ...
    df.to_csv('workdir/sensitivity_analysis/doe.csv')
...
def sobol_analysis(conf_file, data_file):
    ...
    def update_sobol(change):
        ...
        # export
        fig1.write_html('workdir/sensitivity_analysis/sobol_indices_hist.html')
        fig1.write_image('workdir/sensitivity_analysis/sobol_indices_hist.pdf')
        fig2.write_html('workdir/sensitivity_analysis/sobol_second_order.html')
        fig2.write_image('workdir/sensitivity_analysis/sobol_second_order.pdf')
        fig3.write_html('workdir/sensitivity_analysis/sobol_indices_pie.html')
        fig3.write_image('workdir/sensitivity_analysis/sobol_indices_pie.pdf')
        fig4.write_html('workdir/sensitivity_analysis/output_dist.html')
        fig4.write_image('workdir/sensitivity_analysis/output_dist.pdf')
        fig5.write_html('workdir/sensitivity_analysis/parallel_plot.html')
        fig5.write_image('workdir/sensitivity_analysis/parallel_plot.pdf')
...
def morris_analysis(conf_file, data_file):
    ...
    def update_morris(change):
        ...
        with fig2.batch_update():
            ...
            for i in range(len(Si['names'])):
                fig2.add_trace(go.Scatter(x=[Si['mu_star'][i]], y=[Si['sigma'][i]],
                                          mode='markers',
            ...
            fig2.update_layout(
                ...
                yaxis_range=[-0.05, 1.2 * max(0.1 * max(Si['mu_star']), max(Si['sigma']))],
                legend=dict(title='',
                            orientation='h',
                            bordercolor='black',
                            borderwidth=1
            ...
            # export
            fig1.write_html('workdir/sensitivity_analysis/morris_mu.html', include_mathjax='cdn')
            fig1.write_image('workdir/sensitivity_analysis/morris_mu.pdf')
            fig2.write_html('workdir/sensitivity_analysis/morris_mu_sigma.html', include_mathjax='cdn')
            fig2.write_image('workdir/sensitivity_analysis/morris_mu_sigma.pdf')
    ...
    # add input and update buttons interactions
    addinput_button.on_click(add_input)
    update_button.on_click(update_all)  # Run the Monte Carlo with provided parameters, and display the charts
