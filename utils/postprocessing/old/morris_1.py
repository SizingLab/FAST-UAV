...
def doe_morris(x_dict: dict, y_list: List[str], conf_file: str, nt: int = 1024) -> pd.DataFrame:
    ...
    fail_count = prob.model.sub_prob._fail_count if nested_optimization else 0  # count number of failures
    if fail_count > 0:
...
def morris_analysis(conf_file, data_file):
    ...
    # "add input" button
    addinput_button = widgets.Button(description="add parameter", style={'description_width': 'initial'})
    ...
    def update_morris(change):
        ...
        with fig2.batch_update():
            ...
            fig2.data[0].marker = dict(symbol=symbols, color=np.sqrt(Si['mu_star']**2 + Si['sigma']**2))
    ...
    def update_all(change):
        ...
        for inputrow in inputs_array:
            ...
            if inputbox.value is not None:  # not empty input
                ...
                b = var_box.value[1]
                bounds = [a, b]
                x_dict[get_long_name(inputbox.value)[1]] = bounds  # bounds for parameter error distribution
