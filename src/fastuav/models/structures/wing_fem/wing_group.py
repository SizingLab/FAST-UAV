...
class WingStructureFEM(om.Group):
    ...
    def initialize(self):
        ...
        self.options.declare(
            "n_elements", types=int, default=20, desc="Beam elements for the tube_spar_foam model."
        )
        self.options.declare(
            "n_span",
            types=int,
    ...
    def setup(self):
        ...
        self.add_subsystem(
            "load_distribution",
            WingLoadDistribution(
                n_elements=n_load, use_aero_vectors=self.options["use_aero_vectors"]
            ),
            promotes=["*"],
        )
        ...
        if model == "tube_spar_foam":
            self.add_subsystem(
                "fem",
                TubeSparFoamModel(
                    n_elements=self.options["n_elements"], ks_rho=self.options["ks_rho"]
                ),
                promotes=["*"],
            )
