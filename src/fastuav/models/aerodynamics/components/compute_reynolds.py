...
class ComputeUnitReynolds(ExplicitComponent):
    ...
    def setup(self):
        if self.options["low_speed_aero"]:
            ...
            self.add_input(
                "mission:sizing:main_route:cruise:speed:fixedwing", val=np.nan, units="m/s"
            )
    ...
    def compute_partials(self, inputs, partials, discrete_inputs=None):
        from stdatm import Atmosphere as AtmIsa
        ...
        if self.options["low_speed_aero"]:
            ...
            mach = V / a
            _unit_reynolds = rho * V / mu

            # dMach/dV = 1/a
            partials[
                "data:aerodynamics:cruise:mach", "mission:sizing:main_route:cruise:speed:fixedwing"
            ] = 1.0 / a
            ...
            partials[
                "data:aerodynamics:cruise:mach", "mission:sizing:main_route:cruise:altitude"
            ] = -mach * da_dh / a

            # dunit_reynolds/dV = rho/mu
            partials[
                "data:aerodynamics:cruise:unit_reynolds",
                "mission:sizing:main_route:cruise:speed:fixedwing",
            ] = rho / mu
