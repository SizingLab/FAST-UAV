...
@oad.RegisterOpenMDAOSystem("fastuav.aerodynamics.highspeed.legacy.fixedwing")#, domain=ModelDomain.AERODYNAMICS
...
class ParasiticDragConstraintVLM(om.ExplicitComponent):
    ...
    def compute_partials(self, inputs, partials, discrete_inputs=None):
        ...
        partials["optimization:constraints:aerodynamics:CD0:consistency", "data:aerodynamics:CD0"] = ( #, "data:aerodynamics:CD0"#data:aerodynamics:aircraft:cruise:CD0
            - CD_0_guess / CD_0_com ** 2
        )
        partials["optimization:constraints:aerodynamics:CDi:K:consistency", "optimization:variables:aerodynamics:CDi:K:guess"] = (
            1 / k_CD_i
        )
        partials["optimization:constraints:aerodynamics:CDi:K:consistency", "data:aerodynamics:wing:cruise:CDi:k"] = (
...
class WingParasiticDrag(om.ExplicitComponent):
    ...
    def compute_partials(self, inputs, partials, discrete_inputs=None):
        ...
        cf_wing = AirframeAerodynamicsModel.friction_flatplate(V_cruise, c_MAC_w, nu_air, a_air)
        dcf = AirframeAerodynamicsModel.friction_flatplate_derivatives(V_cruise, c_MAC_w, nu_air, a_air)
        ...
        partials[out, "mission:sizing:main_route:cruise:speed:%s" % propulsion_id] = dcf["dV"] * FF_w * 2

        # Atmospheric partials (stdatm derivatives)
        datm = AtmosphereWithPartials(altitude_cruise, dISA, altitude_in_feet=False)
        dnu_dh = datm.partial_kinematic_viscosity_altitude
...
class TailParasiticDrag(om.ExplicitComponent):
    ...
    def compute_partials(self, inputs, partials, discrete_inputs=None):
        ...
        # Compute cf and its derivatives
        cf_tail = AirframeAerodynamicsModel.friction_flatplate(V_cruise, c_MAC_t, nu_air, a_air)
        dcf = AirframeAerodynamicsModel.friction_flatplate_derivatives(V_cruise, c_MAC_t, nu_air, a_air)
        ...
        partials[out, "data:geometry:tail:%s:MAC:length" % tail] = dcf["dL"] * FF_tail * 2 * S_t / S_ref
        ...
        partials[out, "mission:sizing:main_route:cruise:altitude"] = (
            dcf["dnu"] * dnu_dh * FF_tail + cf_tail * dFF_dh
        ) * 2 * S_t / S_ref
        ...
        partials[out, "mission:sizing:dISA"] = (
            dcf["dnu"] * dnu_ddISA * FF_tail + cf_tail * dFF_ddISA
        ) * 2 * S_t / S_ref
...
class FuselageParasiticDrag(om.ExplicitComponent):
    ...
    def compute_partials(self, inputs, partials, discrete_inputs=None):
        ...
        # Compute cf and its derivatives
        cf_fus = AirframeAerodynamicsModel.friction_flatplate(V_cruise, l_fus, nu_air, a_air)
        dcf = AirframeAerodynamicsModel.friction_flatplate_derivatives(V_cruise, l_fus, nu_air, a_air)
        ...
        partials[out, "data:geometry:wing:surface"] = (
            -(cf_fus * S_wet_fus * FF_fus) / (S_ref ** 2)
        )
        ...
        partials[out, "mission:sizing:dISA"] = (
            dcf["dnu"] * dnu_ddISA * S_wet_fus * FF_fus
        ) / S_ref
...
class MaxLiftToDrag(om.ExplicitComponent):
    ...
    def compute_partials(self, inputs, partials, discrete_inputs=None):
        ...
        partials["data:aerodynamics:LD:max", "data:aerodynamics:CD0"] = -0.25 * K**(-0.5) * CD_0**(-1.5)
        partials["data:aerodynamics:LD:max", "data:aerodynamics:CDi:K"] = -0.25 * CD_0**(-0.5) * K**(-1.5)
