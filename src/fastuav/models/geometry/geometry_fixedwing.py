...
class WingGeometry(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        # Wing location
        y_MAC = ((b_w / 6) * (1 + 2 * lambda_w) / (1 + lambda_w))  # y-location of MAC (from wing root, i.e. symmetry axis of the UAV) [m]
    ...
    def compute_partials(self, inputs, partials, discrete_inputs=None):
        ...
        partials[out2_ID, "optimization:variables:weight:mtow:guess"] = 0.5 * np.sqrt(AR_w/S_w) * g / WS      
        partials[out2_ID, "data:geometry:wing:loading"] = 0.5 * np.sqrt(AR_w/S_w) * (-m_uav_guess * g / WS**2)
        partials[out2_ID, "optimization:variables:geometry:wing:AR"] = 0.5 * np.sqrt(S_w / AR_w)

        out3_ID = "data:geometry:wing:root:chord"
        partials[out3_ID, "optimization:variables:weight:mtow:guess"] = 1 / (np.sqrt(S_w * AR_w) * (1 + lambda_w)) * g / WS
        partials[out3_ID, "data:geometry:wing:loading"] = 1 / (np.sqrt(S_w * AR_w) * (1 + lambda_w)) * (-m_uav_guess * g / WS**2)
        partials[out3_ID, "optimization:variables:geometry:wing:AR"] = - np.sqrt(S_w / AR_w**3) / (1 + lambda_w)
        partials[out3_ID, "optimization:variables:geometry:wing:lambda"] = -2 * S_w / b_w / (1 + lambda_w)**2

        out4_ID = "data:geometry:wing:tip:chord"
        partials[out4_ID, "optimization:variables:weight:mtow:guess"] = lambda_w / (np.sqrt(S_w * AR_w) * (1 + lambda_w)) * g / WS
        partials[out4_ID, "data:geometry:wing:loading"] = lambda_w / (np.sqrt(S_w * AR_w) * (1 + lambda_w)) * (-m_uav_guess * g / WS**2)
        partials[out4_ID, "optimization:variables:geometry:wing:AR"] = - lambda_w *np.sqrt(S_w / AR_w**3) / (1 + lambda_w)
        partials[out4_ID, "optimization:variables:geometry:wing:lambda"] = 2 * S_w / b_w /(1 + lambda_w)**2

        out5_ID = "data:geometry:wing:MAC:length"
        partials[out5_ID, "optimization:variables:weight:mtow:guess"] = (2 / 3) * (1 + lambda_w + lambda_w**2) / (1 + lambda_w) / (np.sqrt(S_w * AR_w) * (1 + lambda_w)) * g / WS
        partials[out5_ID, "data:geometry:wing:loading"] = (2 / 3) * (1 + lambda_w + lambda_w**2) / (1 + lambda_w) / (np.sqrt(S_w * AR_w) * (1 + lambda_w)) * (-m_uav_guess * g / WS**2)
        partials[out5_ID, "optimization:variables:geometry:wing:AR"] = (2 / 3) * (1 + lambda_w + lambda_w**2) / (1 + lambda_w)*(- np.sqrt(S_w / AR_w**3) / (1 + lambda_w))
        partials[out5_ID, "optimization:variables:geometry:wing:lambda"] = (2 / 3) * 2 * S_w / b_w *(lambda_w-1)/(1 + lambda_w)**3 

        out6_ID = "data:geometry:wing:root:thickness"
        partials[out6_ID, "optimization:variables:weight:mtow:guess"] = tc_ratio / (np.sqrt(S_w * AR_w) * (1 + lambda_w)) * g / WS
        partials[out6_ID, "data:geometry:wing:loading"] = tc_ratio / (np.sqrt(S_w * AR_w) * (1 + lambda_w)) * (-m_uav_guess * g / WS**2)
        partials[out6_ID, "optimization:variables:geometry:wing:AR"] = - tc_ratio * np.sqrt(S_w / AR_w**3) / (1 + lambda_w)
        partials[out6_ID, "optimization:variables:geometry:wing:lambda"] = - 2 * tc_ratio * S_w / b_w / (1 + lambda_w)**2
        partials[out6_ID, "data:geometry:wing:tc"] = c_root


        out7_ID = "data:geometry:wing:tip:thickness"
        partials[out7_ID, "optimization:variables:weight:mtow:guess"] = tc_ratio * lambda_w / (np.sqrt(S_w * AR_w) * (1 + lambda_w)) * g / WS
        partials[out7_ID, "data:geometry:wing:loading"] = tc_ratio * lambda_w / (np.sqrt(S_w * AR_w) * (1 + lambda_w)) * (-m_uav_guess * g / WS**2)
        partials[out7_ID, "optimization:variables:geometry:wing:AR"] = - tc_ratio * lambda_w * np.sqrt(S_w / AR_w**3) / (1 + lambda_w)
        partials[out7_ID, "optimization:variables:geometry:wing:lambda"] = 2 * tc_ratio * S_w / b_w / (1 + lambda_w)**2
        partials[out7_ID, "data:geometry:wing:tc"] = c_tip


        out8_ID = "data:geometry:wing:MAC:y"
        partials[out8_ID, "optimization:variables:weight:mtow:guess"] = ((0.5 * np.sqrt(AR_w/S_w) * g / WS / 6) * (1 + 2 * lambda_w) / (1 + lambda_w))
        partials[out8_ID, "data:geometry:wing:loading"] = ((0.5 * np.sqrt(AR_w/S_w) * (-m_uav_guess * g / WS**2)/ 6) * (1 + 2 * lambda_w) / (1 + lambda_w))
        partials[out8_ID, "optimization:variables:geometry:wing:AR"] = ((0.5 * np.sqrt(S_w / AR_w) / 6) * (1 + 2 * lambda_w) / (1 + lambda_w))
        partials[out8_ID, "optimization:variables:geometry:wing:lambda"] = (b_w / 6) / (1 + lambda_w)**2
        
        out9_ID = "data:geometry:wing:MAC:LE:x"
        partials[out9_ID, "optimization:variables:weight:mtow:guess"] = k_xw * 0.5 * np.sqrt(AR_w/S_w) * g / WS      
        partials[out9_ID, "data:geometry:wing:loading"] = k_xw * 0.5 * np.sqrt(AR_w/S_w) * (-m_uav_guess * g / WS**2)
        partials[out9_ID, "optimization:variables:geometry:wing:AR"] = k_xw * 0.5 * np.sqrt(S_w / AR_w)
        partials[out9_ID, "optimization:variables:geometry:wing:MAC:LE:x:k"] = b_w

        out10_ID = "data:geometry:wing:MAC:C4:x"
        partials[out10_ID, "optimization:variables:weight:mtow:guess"] = k_xw * 0.5 * np.sqrt(AR_w/S_w) * g / WS + \
                    0.25 * (2 / 3) * (1 + lambda_w + lambda_w**2) / (1 + lambda_w) / (np.sqrt(S_w * AR_w) * (1 + lambda_w)) * g / WS
        partials[out10_ID, "data:geometry:wing:loading"] = k_xw * 0.5 * np.sqrt(AR_w/S_w) * (-m_uav_guess * g / WS**2) + \
                    0.25 * (2 / 3) * (1 + lambda_w + lambda_w**2) / (1 + lambda_w) / (np.sqrt(S_w * AR_w) * (1 + lambda_w)) * (-m_uav_guess * g / WS**2)
        partials[out10_ID, "optimization:variables:geometry:wing:AR"] = k_xw * 0.5 * np.sqrt(S_w / AR_w) + \
                    0.25 * (2 / 3) * (1 + lambda_w + lambda_w**2) / (1 + lambda_w)*(- np.sqrt(S_w / AR_w**3) / (1 + lambda_w))
        partials[out10_ID, "optimization:variables:geometry:wing:lambda"] = 0.25*(2 / 3) * 2 * S_w / b_w *(lambda_w-1)/(1 + lambda_w)**3 
        partials[out10_ID, "optimization:variables:geometry:wing:MAC:LE:x:k"] = b_w 

        out11_ID = "data:geometry:wing:root:LE:x"
        partials[out11_ID, "optimization:variables:weight:mtow:guess"] = k_xw * 0.5 * np.sqrt(AR_w/S_w) * g / WS - \
                    np.tan(sweep_LE) *  ((0.5 * np.sqrt(AR_w/S_w) * g / WS / 6) * (1 + 2 * lambda_w) / (1 + lambda_w))    
        partials[out11_ID, "data:geometry:wing:loading"] = k_xw * 0.5 * np.sqrt(AR_w/S_w) * (-m_uav_guess * g / WS**2)- \
                    np.tan(sweep_LE) *  ((0.5 * np.sqrt(AR_w/S_w) * (-m_uav_guess * g / WS**2)/ 6) * (1 + 2 * lambda_w) / (1 + lambda_w))
        partials[out11_ID, "optimization:variables:geometry:wing:AR"] = k_xw * 0.5 * np.sqrt(S_w / AR_w) - \
                    np.tan(sweep_LE) *  ((0.5 * np.sqrt(S_w / AR_w) / 6) * (1 + 2 * lambda_w) / (1 + lambda_w))
        partials[out11_ID, "optimization:variables:geometry:wing:MAC:LE:x:k"] = b_w
        partials[out11_ID, "optimization:variables:geometry:wing:lambda"] = - np.tan(sweep_LE) * (b_w / 6) / (1 + lambda_w)**2
        partials[out11_ID, "data:geometry:wing:sweep:LE"] = - 1 / np.cos(sweep_LE)**2 * y_MAC
        
        out12_ID = "data:geometry:wing:root:TE:x"
        partials[out12_ID, "optimization:variables:weight:mtow:guess"] = k_xw * 0.5 * np.sqrt(AR_w/S_w) * g / WS - \
                    np.tan(sweep_LE) *  ((0.5 * np.sqrt(AR_w/S_w) * g / WS / 6) * (1 + 2 * lambda_w) / (1 + lambda_w)) + \
                    1 / (np.sqrt(S_w * AR_w) * (1 + lambda_w)) * g / WS  
        partials[out12_ID, "data:geometry:wing:loading"] = k_xw * 0.5 * np.sqrt(AR_w/S_w) * (-m_uav_guess * g / WS**2)- \
                    np.tan(sweep_LE) *  ((0.5 * np.sqrt(AR_w/S_w) * (-m_uav_guess * g / WS**2)/ 6) * (1 + 2 * lambda_w) / (1 + lambda_w)) + \
                    1 / (np.sqrt(S_w * AR_w) * (1 + lambda_w)) * (-m_uav_guess * g / WS**2)
        partials[out12_ID, "optimization:variables:geometry:wing:AR"] = k_xw * 0.5 * np.sqrt(S_w / AR_w) - \
                    np.tan(sweep_LE) *  ((0.5 * np.sqrt(S_w / AR_w) / 6) * (1 + 2 * lambda_w) / (1 + lambda_w)) + \
                    - np.sqrt(S_w / AR_w**3) / (1 + lambda_w)
        partials[out12_ID, "optimization:variables:geometry:wing:MAC:LE:x:k"] = b_w
        partials[out12_ID, "optimization:variables:geometry:wing:lambda"] = - np.tan(sweep_LE) * (b_w / 6) / (1 + lambda_w)**2 - \
        ...
        partials[out13_ID, "optimization:variables:geometry:wing:AR"] = 4*(1 - lambda_w)/(1 + lambda_w) / (1 + (np.tan(sweep_LE) - 4 / AR_w * (1 - lambda_w)/(1 + lambda_w))**2) / AR_w**2
        partials[out13_ID, "optimization:variables:geometry:wing:lambda"] = 4 / AR_w * (2) / (1 + lambda_w)**2 / (1 + (np.tan(sweep_LE) - 4 / AR_w * (1 - lambda_w)/(1 + lambda_w))**2)
        partials[out13_ID, "data:geometry:wing:sweep:LE"] = 1 / np.cos(sweep_LE)**2 / (1 + (np.tan(sweep_LE) - 4 / AR_w * (1 - lambda_w)/(1 + lambda_w))**2)
...
class HorizontalTailGeometry(om.ExplicitComponent):
    ...
    def compute_partials(self, inputs, partials, discrete_inputs=None):
        ...
        # --- Output 1: l_ht ---
        partials["data:geometry:tail:horizontal:arm", "optimization:variables:geometry:tail:horizontal:arm:k"] = dl_ht_dk_ht
        partials["data:geometry:tail:horizontal:arm", "data:geometry:wing:span"] = dl_ht_db_w
 
        # --- Output 2: S_ht ---
        partials["data:geometry:tail:horizontal:surface", "data:geometry:tail:horizontal:coefficient"] = dS_ht_dV_ht
        partials["data:geometry:tail:horizontal:surface", "data:geometry:wing:surface"] = dS_ht_dS_w
        partials["data:geometry:tail:horizontal:surface", "data:geometry:wing:MAC:length"] = dS_ht_dc_MAC_w
        partials["data:geometry:tail:horizontal:surface", "optimization:variables:geometry:tail:horizontal:arm:k"] = dS_ht_dk_ht
        partials["data:geometry:tail:horizontal:surface", "data:geometry:wing:span"] = dS_ht_db_w
 
        # --- Output 3: b_ht ---
        partials["data:geometry:tail:horizontal:span", "optimization:variables:geometry:tail:horizontal:AR"] = db_ht_dAR_ht
        partials["data:geometry:tail:horizontal:span", "data:geometry:tail:horizontal:coefficient"] = db_ht_dV_ht
        partials["data:geometry:tail:horizontal:span", "data:geometry:wing:surface"] = db_ht_dS_w
        partials["data:geometry:tail:horizontal:span", "data:geometry:wing:MAC:length"] = db_ht_dc_MAC_w
        partials["data:geometry:tail:horizontal:span", "optimization:variables:geometry:tail:horizontal:arm:k"] = db_ht_dk_ht
        partials["data:geometry:tail:horizontal:span", "data:geometry:wing:span"] = db_ht_db_w
 
        # --- Output 4: c_root_ht ---
        partials["data:geometry:tail:horizontal:root:chord", "optimization:variables:geometry:tail:horizontal:AR"] = dc_root_dAR_ht
        partials["data:geometry:tail:horizontal:root:chord", "data:geometry:tail:horizontal:coefficient"] = dc_root_dV_ht
        partials["data:geometry:tail:horizontal:root:chord", "data:geometry:wing:surface"] = dc_root_dS_w
        partials["data:geometry:tail:horizontal:root:chord", "data:geometry:wing:MAC:length"] = dc_root_dc_MAC_w
        partials["data:geometry:tail:horizontal:root:chord", "optimization:variables:geometry:tail:horizontal:arm:k"] = dc_root_dk_ht
        partials["data:geometry:tail:horizontal:root:chord", "data:geometry:wing:span"] = dc_root_db_w
        partials["data:geometry:tail:horizontal:root:chord", "data:geometry:tail:horizontal:lambda"] = dc_root_dlambda
 
        # --- Output 5: c_tip_ht = lambda * c_root_ht ---
        partials["data:geometry:tail:horizontal:tip:chord", "optimization:variables:geometry:tail:horizontal:AR"] = lmbda_ht * dc_root_dAR_ht
        partials["data:geometry:tail:horizontal:tip:chord", "data:geometry:tail:horizontal:coefficient"] = lmbda_ht * dc_root_dV_ht
        partials["data:geometry:tail:horizontal:tip:chord", "data:geometry:wing:surface"] = lmbda_ht * dc_root_dS_w
        partials["data:geometry:tail:horizontal:tip:chord", "data:geometry:wing:MAC:length"] = lmbda_ht * dc_root_dc_MAC_w
        partials["data:geometry:tail:horizontal:tip:chord", "optimization:variables:geometry:tail:horizontal:arm:k"] = lmbda_ht * dc_root_dk_ht
        partials["data:geometry:tail:horizontal:tip:chord", "data:geometry:wing:span"] = lmbda_ht * dc_root_db_w
        partials["data:geometry:tail:horizontal:tip:chord", "data:geometry:tail:horizontal:lambda"] = c_root_ht + lmbda_ht * dc_root_dlambda
 
        # --- Output 6: c_MAC_ht ---
        partials["data:geometry:tail:horizontal:MAC:length", "optimization:variables:geometry:tail:horizontal:AR"] = dc_MAC_dAR_ht
        partials["data:geometry:tail:horizontal:MAC:length", "data:geometry:tail:horizontal:coefficient"] = dc_MAC_dV_ht
        partials["data:geometry:tail:horizontal:MAC:length", "data:geometry:wing:surface"] = dc_MAC_dS_w
        partials["data:geometry:tail:horizontal:MAC:length", "data:geometry:wing:MAC:length"] = dc_MAC_dc_MAC_w
        partials["data:geometry:tail:horizontal:MAC:length", "optimization:variables:geometry:tail:horizontal:arm:k"] = dc_MAC_dk_ht
        partials["data:geometry:tail:horizontal:MAC:length", "data:geometry:wing:span"] = dc_MAC_db_w
        partials["data:geometry:tail:horizontal:MAC:length", "data:geometry:tail:horizontal:lambda"] = dc_MAC_dlambda
 
        # --- Output 7: t_root_ht = c_root_ht * tc_ratio ---
        partials["data:geometry:tail:horizontal:root:thickness", "optimization:variables:geometry:tail:horizontal:AR"] = tc_ratio * dc_root_dAR_ht
        partials["data:geometry:tail:horizontal:root:thickness", "data:geometry:tail:horizontal:coefficient"] = tc_ratio * dc_root_dV_ht
        partials["data:geometry:tail:horizontal:root:thickness", "data:geometry:wing:surface"] = tc_ratio * dc_root_dS_w
        partials["data:geometry:tail:horizontal:root:thickness", "data:geometry:wing:MAC:length"] = tc_ratio * dc_root_dc_MAC_w
        partials["data:geometry:tail:horizontal:root:thickness", "optimization:variables:geometry:tail:horizontal:arm:k"] = tc_ratio * dc_root_dk_ht
        partials["data:geometry:tail:horizontal:root:thickness", "data:geometry:wing:span"] = tc_ratio * dc_root_db_w
        partials["data:geometry:tail:horizontal:root:thickness", "data:geometry:tail:horizontal:lambda"] = tc_ratio * dc_root_dlambda
        partials["data:geometry:tail:horizontal:root:thickness", "data:geometry:wing:tc"] = c_root_ht
 
        # --- Output 8: t_tip_ht = c_tip_ht * tc_ratio ---
        partials["data:geometry:tail:horizontal:tip:thickness", "optimization:variables:geometry:tail:horizontal:AR"] = tc_ratio * lmbda_ht * dc_root_dAR_ht
        partials["data:geometry:tail:horizontal:tip:thickness", "data:geometry:tail:horizontal:coefficient"] = tc_ratio * lmbda_ht * dc_root_dV_ht
        partials["data:geometry:tail:horizontal:tip:thickness", "data:geometry:wing:surface"] = tc_ratio * lmbda_ht * dc_root_dS_w
        partials["data:geometry:tail:horizontal:tip:thickness", "data:geometry:wing:MAC:length"] = tc_ratio * lmbda_ht * dc_root_dc_MAC_w
        partials["data:geometry:tail:horizontal:tip:thickness", "optimization:variables:geometry:tail:horizontal:arm:k"] = tc_ratio * lmbda_ht * dc_root_dk_ht
        partials["data:geometry:tail:horizontal:tip:thickness", "data:geometry:wing:span"] = tc_ratio * lmbda_ht * dc_root_db_w
        partials["data:geometry:tail:horizontal:tip:thickness", "data:geometry:tail:horizontal:lambda"] = tc_ratio * (c_root_ht + lmbda_ht * dc_root_dlambda)
        ...
        dy_MAC_dlambda = (b_ht / 6) * (2 * (1 + lmbda_ht) - (1 + 2 * lmbda_ht)) / (1 + lmbda_ht) ** 2
        partials["data:geometry:tail:horizontal:MAC:y", "optimization:variables:geometry:tail:horizontal:AR"] = dy_MAC_db_ht * db_ht_dAR_ht
        partials["data:geometry:tail:horizontal:MAC:y", "data:geometry:tail:horizontal:coefficient"] = dy_MAC_db_ht * db_ht_dV_ht
        partials["data:geometry:tail:horizontal:MAC:y", "data:geometry:wing:surface"] = dy_MAC_db_ht * db_ht_dS_w
        partials["data:geometry:tail:horizontal:MAC:y", "data:geometry:wing:MAC:length"] = dy_MAC_db_ht * db_ht_dc_MAC_w
        partials["data:geometry:tail:horizontal:MAC:y", "optimization:variables:geometry:tail:horizontal:arm:k"] = dy_MAC_db_ht * db_ht_dk_ht
        partials["data:geometry:tail:horizontal:MAC:y", "data:geometry:wing:span"] = dy_MAC_db_ht * db_ht_db_w
        partials["data:geometry:tail:horizontal:MAC:y", "data:geometry:tail:horizontal:lambda"] = dy_MAC_dlambda
 
        # --- Output 10: x_MAC_c4_ht = x_MAC_c4_w + l_ht ---
        partials["data:geometry:tail:horizontal:MAC:C4:x", "data:geometry:wing:MAC:C4:x"] = 1.0
        partials["data:geometry:tail:horizontal:MAC:C4:x", "optimization:variables:geometry:tail:horizontal:arm:k"] = dl_ht_dk_ht
        ...
        partials["data:geometry:tail:horizontal:MAC:LE:x", "optimization:variables:geometry:tail:horizontal:AR"] = -0.25 * dc_MAC_dAR_ht
        partials["data:geometry:tail:horizontal:MAC:LE:x", "data:geometry:tail:horizontal:coefficient"] = -0.25 * dc_MAC_dV_ht
        partials["data:geometry:tail:horizontal:MAC:LE:x", "data:geometry:wing:surface"] = -0.25 * dc_MAC_dS_w
        partials["data:geometry:tail:horizontal:MAC:LE:x", "data:geometry:wing:MAC:length"] = -0.25 * dc_MAC_dc_MAC_w
        partials["data:geometry:tail:horizontal:MAC:LE:x", "optimization:variables:geometry:tail:horizontal:arm:k"] = dl_ht_dk_ht - 0.25 * dc_MAC_dk_ht
        partials["data:geometry:tail:horizontal:MAC:LE:x", "data:geometry:wing:span"] = dl_ht_db_w - 0.25 * dc_MAC_db_w
        partials["data:geometry:tail:horizontal:MAC:LE:x", "data:geometry:tail:horizontal:lambda"] = -0.25 * dc_MAC_dlambda
        ...
        partials["data:geometry:tail:horizontal:root:LE:x", "optimization:variables:geometry:tail:horizontal:AR"] = -0.25 * dc_MAC_dAR_ht
        partials["data:geometry:tail:horizontal:root:LE:x", "data:geometry:tail:horizontal:coefficient"] = -0.25 * dc_MAC_dV_ht
        partials["data:geometry:tail:horizontal:root:LE:x", "data:geometry:wing:surface"] = -0.25 * dc_MAC_dS_w
        partials["data:geometry:tail:horizontal:root:LE:x", "data:geometry:wing:MAC:length"] = -0.25 * dc_MAC_dc_MAC_w
        partials["data:geometry:tail:horizontal:root:LE:x", "optimization:variables:geometry:tail:horizontal:arm:k"] = dl_ht_dk_ht - 0.25 * dc_MAC_dk_ht
        partials["data:geometry:tail:horizontal:root:LE:x", "data:geometry:wing:span"] = dl_ht_db_w - 0.25 * dc_MAC_db_w
        partials["data:geometry:tail:horizontal:root:LE:x", "data:geometry:tail:horizontal:lambda"] = -0.25 * dc_MAC_dlambda
        ...
        partials["data:geometry:tail:horizontal:root:TE:x", "optimization:variables:geometry:tail:horizontal:AR"] = -0.25 * dc_MAC_dAR_ht + dc_root_dAR_ht
        partials["data:geometry:tail:horizontal:root:TE:x", "data:geometry:tail:horizontal:coefficient"] = -0.25 * dc_MAC_dV_ht + dc_root_dV_ht
        partials["data:geometry:tail:horizontal:root:TE:x", "data:geometry:wing:surface"] = -0.25 * dc_MAC_dS_w + dc_root_dS_w
        partials["data:geometry:tail:horizontal:root:TE:x", "data:geometry:wing:MAC:length"] = -0.25 * dc_MAC_dc_MAC_w + dc_root_dc_MAC_w
        partials["data:geometry:tail:horizontal:root:TE:x", "optimization:variables:geometry:tail:horizontal:arm:k"] = dl_ht_dk_ht - 0.25 * dc_MAC_dk_ht + dc_root_dk_ht
        partials["data:geometry:tail:horizontal:root:TE:x", "data:geometry:wing:span"] = dl_ht_db_w - 0.25 * dc_MAC_db_w + dc_root_db_w
        partials["data:geometry:tail:horizontal:root:TE:x", "data:geometry:tail:horizontal:lambda"] = -0.25 * dc_MAC_dlambda + dc_root_dlambda
...
class VerticalTailGeometry(om.ExplicitComponent):
    ...
    def compute_partials(self, inputs, partials, discrete_inputs=None):
        ...
        # --- Output 1: l_vt ---
        partials["data:geometry:tail:vertical:arm", "data:geometry:tail:horizontal:arm"] = dl_vt_dl_ht
 
        # --- Output 2: S_vt ---
        partials["data:geometry:tail:vertical:surface", "data:geometry:tail:vertical:coefficient"] = dS_vt_dV_vt
        ...
        partials["data:geometry:tail:vertical:surface", "data:geometry:tail:horizontal:arm"] = dS_vt_dl_ht
 
        # --- Output 3: b_vt ---
        partials["data:geometry:tail:vertical:span", "optimization:variables:geometry:tail:vertical:AR"] = db_vt_dAR_vt
        partials["data:geometry:tail:vertical:span", "data:geometry:tail:vertical:coefficient"] = db_vt_dV_vt
        ...
        partials["data:geometry:tail:vertical:span", "data:geometry:tail:horizontal:arm"] = db_vt_dl_ht
 
        # --- Output 4: c_root_vt ---
        partials["data:geometry:tail:vertical:root:chord", "optimization:variables:geometry:tail:vertical:AR"] = dc_root_dAR_vt
        partials["data:geometry:tail:vertical:root:chord", "data:geometry:tail:vertical:coefficient"] = dc_root_dV_vt
        partials["data:geometry:tail:vertical:root:chord", "data:geometry:wing:surface"] = dc_root_dS_w
        partials["data:geometry:tail:vertical:root:chord", "data:geometry:wing:span"] = dc_root_db_w
        partials["data:geometry:tail:vertical:root:chord", "data:geometry:tail:horizontal:arm"] = dc_root_dl_ht
        partials["data:geometry:tail:vertical:root:chord", "data:geometry:tail:vertical:lambda"] = dc_root_dlambda
 
        # --- Output 5: c_tip_vt = lambda * c_root_vt ---
        partials["data:geometry:tail:vertical:tip:chord", "optimization:variables:geometry:tail:vertical:AR"] = lmbda_vt * dc_root_dAR_vt
        partials["data:geometry:tail:vertical:tip:chord", "data:geometry:tail:vertical:coefficient"] = lmbda_vt * dc_root_dV_vt
        partials["data:geometry:tail:vertical:tip:chord", "data:geometry:wing:surface"] = lmbda_vt * dc_root_dS_w
        partials["data:geometry:tail:vertical:tip:chord", "data:geometry:wing:span"] = lmbda_vt * dc_root_db_w
        partials["data:geometry:tail:vertical:tip:chord", "data:geometry:tail:horizontal:arm"] = lmbda_vt * dc_root_dl_ht
        partials["data:geometry:tail:vertical:tip:chord", "data:geometry:tail:vertical:lambda"] = c_root_vt + lmbda_vt * dc_root_dlambda
 
        # --- Output 6: c_MAC_vt ---
        partials["data:geometry:tail:vertical:MAC:length", "optimization:variables:geometry:tail:vertical:AR"] = dc_MAC_dAR_vt
        partials["data:geometry:tail:vertical:MAC:length", "data:geometry:tail:vertical:coefficient"] = dc_MAC_dV_vt
        partials["data:geometry:tail:vertical:MAC:length", "data:geometry:wing:surface"] = dc_MAC_dS_w
        partials["data:geometry:tail:vertical:MAC:length", "data:geometry:wing:span"] = dc_MAC_db_w
        partials["data:geometry:tail:vertical:MAC:length", "data:geometry:tail:horizontal:arm"] = dc_MAC_dl_ht
        partials["data:geometry:tail:vertical:MAC:length", "data:geometry:tail:vertical:lambda"] = dc_MAC_dlambda
 
        # --- Output 7: t_root_vt = c_root_vt * tc_ratio ---
        partials["data:geometry:tail:vertical:root:thickness", "optimization:variables:geometry:tail:vertical:AR"] = tc_ratio * dc_root_dAR_vt
        partials["data:geometry:tail:vertical:root:thickness", "data:geometry:tail:vertical:coefficient"] = tc_ratio * dc_root_dV_vt
        partials["data:geometry:tail:vertical:root:thickness", "data:geometry:wing:surface"] = tc_ratio * dc_root_dS_w
        partials["data:geometry:tail:vertical:root:thickness", "data:geometry:wing:span"] = tc_ratio * dc_root_db_w
        partials["data:geometry:tail:vertical:root:thickness", "data:geometry:tail:horizontal:arm"] = tc_ratio * dc_root_dl_ht
        partials["data:geometry:tail:vertical:root:thickness", "data:geometry:tail:vertical:lambda"] = tc_ratio * dc_root_dlambda
        partials["data:geometry:tail:vertical:root:thickness", "data:geometry:wing:tc"] = c_root_vt
 
        # --- Output 8: t_tip_vt = c_tip_vt * tc_ratio ---
        partials["data:geometry:tail:vertical:tip:thickness", "optimization:variables:geometry:tail:vertical:AR"] = tc_ratio * lmbda_vt * dc_root_dAR_vt
        partials["data:geometry:tail:vertical:tip:thickness", "data:geometry:tail:vertical:coefficient"] = tc_ratio * lmbda_vt * dc_root_dV_vt
        partials["data:geometry:tail:vertical:tip:thickness", "data:geometry:wing:surface"] = tc_ratio * lmbda_vt * dc_root_dS_w
        partials["data:geometry:tail:vertical:tip:thickness", "data:geometry:wing:span"] = tc_ratio * lmbda_vt * dc_root_db_w
        partials["data:geometry:tail:vertical:tip:thickness", "data:geometry:tail:horizontal:arm"] = tc_ratio * lmbda_vt * dc_root_dl_ht
        partials["data:geometry:tail:vertical:tip:thickness", "data:geometry:tail:vertical:lambda"] = tc_ratio * (c_root_vt + lmbda_vt * dc_root_dlambda)
        ...
        dz_MAC_dlambda = (b_vt / 6) * (2 * (1 + lmbda_vt) - (1 + 2 * lmbda_vt)) / (1 + lmbda_vt) ** 2
        partials["data:geometry:tail:vertical:MAC:z", "optimization:variables:geometry:tail:vertical:AR"] = dz_MAC_db_vt * db_vt_dAR_vt
        partials["data:geometry:tail:vertical:MAC:z", "data:geometry:tail:vertical:coefficient"] = dz_MAC_db_vt * db_vt_dV_vt
        partials["data:geometry:tail:vertical:MAC:z", "data:geometry:wing:surface"] = dz_MAC_db_vt * db_vt_dS_w
        partials["data:geometry:tail:vertical:MAC:z", "data:geometry:wing:span"] = dz_MAC_db_vt * db_vt_db_w
        partials["data:geometry:tail:vertical:MAC:z", "data:geometry:tail:horizontal:arm"] = dz_MAC_db_vt * db_vt_dl_ht
        partials["data:geometry:tail:vertical:MAC:z", "data:geometry:tail:vertical:lambda"] = dz_MAC_dlambda
 
        # --- Output 10: x_MAC_c4_vt = x_MAC_c4_w + l_vt ---
        partials["data:geometry:tail:vertical:MAC:C4:x", "data:geometry:wing:MAC:C4:x"] = 1.0
        partials["data:geometry:tail:vertical:MAC:C4:x", "data:geometry:tail:horizontal:arm"] = dl_vt_dl_ht
        ...
        partials["data:geometry:tail:vertical:MAC:LE:x", "optimization:variables:geometry:tail:vertical:AR"] = -0.25 * dc_MAC_dAR_vt
        partials["data:geometry:tail:vertical:MAC:LE:x", "data:geometry:tail:vertical:coefficient"] = -0.25 * dc_MAC_dV_vt
        partials["data:geometry:tail:vertical:MAC:LE:x", "data:geometry:wing:surface"] = -0.25 * dc_MAC_dS_w
        partials["data:geometry:tail:vertical:MAC:LE:x", "data:geometry:wing:span"] = -0.25 * dc_MAC_db_w
        partials["data:geometry:tail:vertical:MAC:LE:x", "data:geometry:tail:horizontal:arm"] = dl_vt_dl_ht - 0.25 * dc_MAC_dl_ht
        partials["data:geometry:tail:vertical:MAC:LE:x", "data:geometry:tail:vertical:lambda"] = -0.25 * dc_MAC_dlambda
 
        # --- Output 12: x_root_LE_vt = x_MAC_LE_vt ---
        partials["data:geometry:tail:vertical:root:LE:x", "data:geometry:wing:MAC:C4:x"] = 1.0
        partials["data:geometry:tail:vertical:root:LE:x", "optimization:variables:geometry:tail:vertical:AR"] = -0.25 * dc_MAC_dAR_vt
        partials["data:geometry:tail:vertical:root:LE:x", "data:geometry:tail:vertical:coefficient"] = -0.25 * dc_MAC_dV_vt
        partials["data:geometry:tail:vertical:root:LE:x", "data:geometry:wing:surface"] = -0.25 * dc_MAC_dS_w
        partials["data:geometry:tail:vertical:root:LE:x", "data:geometry:wing:span"] = -0.25 * dc_MAC_db_w
        partials["data:geometry:tail:vertical:root:LE:x", "data:geometry:tail:horizontal:arm"] = dl_vt_dl_ht - 0.25 * dc_MAC_dl_ht
        partials["data:geometry:tail:vertical:root:LE:x", "data:geometry:tail:vertical:lambda"] = -0.25 * dc_MAC_dlambda
        ...
        partials["data:geometry:tail:vertical:root:TE:x", "optimization:variables:geometry:tail:vertical:AR"] = -0.25 * dc_MAC_dAR_vt + dc_root_dAR_vt
        partials["data:geometry:tail:vertical:root:TE:x", "data:geometry:tail:vertical:coefficient"] = -0.25 * dc_MAC_dV_vt + dc_root_dV_vt
        partials["data:geometry:tail:vertical:root:TE:x", "data:geometry:wing:surface"] = -0.25 * dc_MAC_dS_w + dc_root_dS_w
        partials["data:geometry:tail:vertical:root:TE:x", "data:geometry:wing:span"] = -0.25 * dc_MAC_db_w + dc_root_db_w
        partials["data:geometry:tail:vertical:root:TE:x", "data:geometry:tail:horizontal:arm"] = dl_vt_dl_ht - 0.25 * dc_MAC_dl_ht + dc_root_dl_ht
        partials["data:geometry:tail:vertical:root:TE:x", "data:geometry:tail:vertical:lambda"] = -0.25 * dc_MAC_dlambda + dc_root_dlambda
...
class FuselageGeometry(om.ExplicitComponent):
    ...
    def compute_partials(self, inputs, partials, discrete_inputs=None):
        ...
        # --- Output 1: l_fus ---
        partials["data:geometry:fuselage:length", "data:geometry:tail:horizontal:root:TE:x"] = dl_fus_dx_ht

        # --- Output 2: l_nose ---
        partials["data:geometry:fuselage:length:nose", "data:geometry:tail:horizontal:root:TE:x"] = dl_nose_dx_ht
        partials["data:geometry:fuselage:length:nose", "data:geometry:fuselage:fineness"] = dl_nose_dlambda

        # --- Output 3: l_mid ---
        partials["data:geometry:fuselage:length:mid", "data:geometry:tail:horizontal:root:TE:x"] = dl_mid_dx_ht
        partials["data:geometry:fuselage:length:mid", "data:geometry:wing:root:TE:x"] = dl_mid_dx_w
        partials["data:geometry:fuselage:length:mid", "data:geometry:fuselage:fineness"] = dl_mid_dlambda

        # --- Output 4: l_rear ---
        partials["data:geometry:fuselage:length:rear", "data:geometry:tail:horizontal:root:TE:x"] = dl_rear_dx_ht
        partials["data:geometry:fuselage:length:rear", "data:geometry:wing:root:TE:x"] = dl_rear_dx_w

        # --- Output 5: d_fus_mid ---
        partials["data:geometry:fuselage:diameter:mid", "data:geometry:tail:horizontal:root:TE:x"] = dd_mid_dx_ht
        partials["data:geometry:fuselage:diameter:mid", "data:geometry:fuselage:fineness"] = dd_mid_dlambda

        # --- Output 6: d_fus_tip ---
        partials["data:geometry:fuselage:diameter:tip", "data:geometry:tail:horizontal:root:TE:x"] = dd_tip_dx_ht
        partials["data:geometry:fuselage:diameter:tip", "data:geometry:fuselage:fineness"] = dd_tip_dlambda
        partials["data:geometry:fuselage:diameter:tip", "data:geometry:fuselage:diameter:k"] = dd_tip_dk_df

        # --- Output 7: S_fus ---
        partials["data:geometry:fuselage:surface", "data:geometry:tail:horizontal:root:TE:x"] = dS_fus_dx_ht
        partials["data:geometry:fuselage:surface", "data:geometry:wing:root:TE:x"] = dS_fus_dx_w
        partials["data:geometry:fuselage:surface", "data:geometry:fuselage:fineness"] = dS_fus_dlambda
        partials["data:geometry:fuselage:surface", "data:geometry:fuselage:diameter:k"] = dS_fus_dk_df

        # --- Output 8: S_nose ---
        partials["data:geometry:fuselage:surface:nose", "data:geometry:tail:horizontal:root:TE:x"] = dS_nose_dx_ht
        partials["data:geometry:fuselage:surface:nose", "data:geometry:fuselage:fineness"] = dS_nose_dlambda

        # --- Output 9: S_mid ---
        partials["data:geometry:fuselage:surface:mid", "data:geometry:tail:horizontal:root:TE:x"] = dS_mid_dx_ht
        partials["data:geometry:fuselage:surface:mid", "data:geometry:wing:root:TE:x"] = dS_mid_dx_w
        partials["data:geometry:fuselage:surface:mid", "data:geometry:fuselage:fineness"] = dS_mid_dlambda

        # --- Output 10: S_rear ---
        partials["data:geometry:fuselage:surface:rear", "data:geometry:tail:horizontal:root:TE:x"] = dS_rear_dx_ht
        partials["data:geometry:fuselage:surface:rear", "data:geometry:wing:root:TE:x"] = dS_rear_dx_w
