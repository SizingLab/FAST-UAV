...
class Endurance(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input(
            "data:propulsion:%s:battery:capacity" % propulsion_id,
            val=np.nan,
            units="A*s",
        )
        self.add_input(
    ...
    def compute(self, inputs, outputs):
        ...
        t_max = (
            C_ratio * C_bat / I_bat if I_bat > 0 else 0.0
        )  # [s] Max. cruise flight time at design payload
    ...
    def compute_partials(self, inputs, partials, discrete_inputs=None):
        ...
        ] = C_bat / I_bat / 60.0 if I_bat > 0 else 0.0
        partials[
            "data:performance:endurance:%s" % phase_name,
            "data:propulsion:%s:battery:capacity" % propulsion_id,
        ] = C_ratio / I_bat / 60.0 if I_bat > 0 else 0.0
        partials[
            "data:performance:endurance:%s" % phase_name,
            "data:propulsion:%s:battery:current:%s" % (propulsion_id, phase_name),
        ...
        if phase_name != HOVER_TAG:
            ...
            partials[
                "data:performance:range:%s" % phase_name,
                "data:propulsion:%s:battery:DoD:max" % propulsion_id,
            ] = C_bat / I_bat * V if I_bat > 0 else 0.0
            partials[
                "data:performance:range:%s" % phase_name,
                "data:propulsion:%s:battery:capacity" % propulsion_id,
            ] = C_ratio / I_bat * V if I_bat > 0 else 0.0
            partials[
                "data:performance:range:%s" % phase_name,
                "data:propulsion:%s:battery:current:%s" % (propulsion_id, phase_name),
            ] = -V * C_ratio * C_bat / I_bat**2 if I_bat > 0 else 0.0
