...
class Mission(om.Group):
    ...
    def setup(self):
        ...
        route_1.add_subsystem("route", RouteComponent(route='_1', segments_list=['climb', 'forward', 'hover']), promotes=['*'])
        ...
        route_2.add_subsystem("route", RouteComponent(route='_2', segments_list=['climb', 'forward', 'hover']), promotes=['*'])
...
class ComputeTOW(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('mission:concordia_study:route%s:payload:mass' % self.options['route'], val=np.nan, units='kg')
        self.add_output('mission:concordia_study:route%s:tow' % self.options['route'], units='kg')
    ...
    def compute(self, inputs, outputs):
        ...
        m_pay_mission = inputs['mission:concordia_study:route%s:payload:mass' % self.options['route']]
...
class ClimbSegment(om.ExplicitComponent):
    ...
    def setup(self):
        # System parameters
        self.add_input('mission:concordia_study:route%s:tow' % self.options['route'], val=np.nan, units='kg')
        ...
        # Mission parameters
        self.add_input('mission:concordia_study:route%s:climb:height' % self.options['route'], val=np.nan, units='m')
        ...
        # Outputs: energy
        self.add_output('mission:concordia_study:route%s:climb:duration' % self.options['route'], units='min')
        self.add_output('mission:concordia_study:route%s:climb:energy:propulsion' % self.options['route'], units='kJ')
        self.add_output('mission:concordia_study:route%s:climb:energy:payload' % self.options['route'], units='kJ')
        self.add_output('mission:concordia_study:route%s:climb:energy:avionics' % self.options['route'], units='kJ')
        self.add_output('mission:concordia_study:route%s:climb:energy' % self.options['route'], units='kJ')
    ...
    def compute(self, inputs, outputs):
        ...
        D_cl = inputs['mission:concordia_study:route%s:climb:height' % self.options['route']]
        ...
        outputs['mission:concordia_study:route%s:climb:duration' % self.options['route']] = t_cl / 60  # [min]
        outputs['mission:concordia_study:route%s:climb:energy:propulsion' % self.options['route']] = E_cl_pro / 1000  # [kJ]
        outputs['mission:concordia_study:route%s:climb:energy:payload' % self.options['route']] = E_payload / 1000  # [kJ]
        outputs['mission:concordia_study:route%s:climb:energy:avionics' % self.options['route']] = E_avionics / 1000  # [kJ]
        outputs['mission:concordia_study:route%s:climb:energy' % self.options['route']] = (E_cl_pro + E_payload + E_avionics) / 1000  # [kJ]
...
class HoverSegment(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('data:motor:torque:coefficient', val=np.nan, units='N*m/A')
        # Mission parameters
        self.add_input('mission:concordia_study:route%s:hover:duration' % self.options['route'], val=np.nan, units='min')
        self.add_input('data:ESC:efficiency', val=np.nan, units=None)
        self.add_input('specifications:payload:power', val=.0, units='W')
        self.add_input('data:avionics:power', val=.0, units='W')
        # Outputs: energy
        self.add_output('mission:concordia_study:route%s:hover:energy:propulsion' % self.options['route'], units='kJ')
        self.add_output('mission:concordia_study:route%s:hover:energy:payload' % self.options['route'], units='kJ')
        self.add_output('mission:concordia_study:route%s:hover:energy:avionics' % self.options['route'], units='kJ')
        self.add_output('mission:concordia_study:route%s:hover:energy' % self.options['route'], units='kJ')
    ...
    def compute(self, inputs, outputs):
        ...
        t_hov = inputs['mission:concordia_study:route%s:hover:duration' % self.options['route']] * 60  # [s]
        eta_ESC = inputs['data:ESC:efficiency']
        P_payload = inputs['specifications:payload:power']
        P_avionics = inputs['data:avionics:power']
        ...
        # Energy consumption
        E_hover_pro = (P_el * Npro) / eta_ESC * t_hov  # [J] consumed energy for propulsion
        ...
        outputs['mission:concordia_study:route%s:hover:energy:propulsion' % self.options['route']] = E_hover_pro / 1000  # [kJ]
        outputs['mission:concordia_study:route%s:hover:energy:payload' % self.options['route']] = E_payload / 1000  # [kJ]
        outputs['mission:concordia_study:route%s:hover:energy:avionics' % self.options['route']] = E_avionics / 1000  # [kJ]
        outputs['mission:concordia_study:route%s:hover:energy' % self.options['route']] = (E_hover_pro + E_payload + E_avionics) / 1000  # [kJ]
...
class ForwardSegment(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('data:motor:torque:coefficient', val=np.nan, units='N*m/A')
        # Mission parameters
        self.add_input('mission:concordia_study:route%s:forward:range' % self.options['route'], val=np.nan, units='m')
        self.add_input('mission:sizing_mission:forward:speed', val=np.nan, units='m/s')
        self.add_input('data:ESC:efficiency', val=np.nan, units=None)
        self.add_input('specifications:payload:power', val=.0, units='W')
        self.add_input('data:avionics:power', val=.0, units='W')
        # Outputs: energy
        self.add_output('mission:concordia_study:route%s:forward:duration' % self.options['route'], units='min')
        self.add_output('mission:concordia_study:route%s:forward:energy:propulsion' % self.options['route'], units='kJ')
        self.add_output('mission:concordia_study:route%s:forward:energy:payload' % self.options['route'], units='kJ')
        self.add_output('mission:concordia_study:route%s:forward:energy:avionics' % self.options['route'], units='kJ')
        self.add_output('mission:concordia_study:route%s:forward:energy' % self.options['route'], units='kJ')
    ...
    def compute(self, inputs, outputs):
        ...
        D_ff = inputs['mission:concordia_study:route%s:forward:range' % self.options['route']]
        V_ff = inputs['mission:sizing_mission:forward:speed']  # TODO: compute optimal speed
        t_ff = D_ff / V_ff  # [s]
        eta_ESC = inputs['data:ESC:efficiency']
        P_payload = inputs['specifications:payload:power']
        P_avionics = inputs['data:avionics:power']
        ...
        outputs['mission:concordia_study:route%s:forward:duration' % self.options['route']] = t_ff / 60  # [min]
        outputs['mission:concordia_study:route%s:forward:energy:propulsion' % self.options['route']] = E_ff_pro / 1000  # [kJ]
        outputs['mission:concordia_study:route%s:forward:energy:payload' % self.options['route']] = E_payload / 1000  # [kJ]
        outputs['mission:concordia_study:route%s:forward:energy:avionics' % self.options['route']] = E_avionics / 1000  # [kJ]
        outputs['mission:concordia_study:route%s:forward:energy' % self.options['route']] = (E_ff_pro + E_payload + E_avionics) / 1000  # [kJ]
...
class RouteComponent(om.ExplicitComponent):
    ...
    def setup(self):
        for segment in self.options['segments_list']:
            self.add_input('mission:concordia_study:route%s:%s:energy' % (self.options['route'], segment), val=np.nan,
                           units='kJ')
            self.add_input('mission:concordia_study:route%s:%s:energy:propulsion' % (self.options['route'], segment),
                           val=np.nan, units='kJ')
            self.add_input('mission:concordia_study:route%s:%s:energy:payload' % (self.options['route'], segment),
                           val=np.nan, units='kJ')
            self.add_input('mission:concordia_study:route%s:%s:energy:avionics' % (self.options['route'], segment),
                           val=np.nan, units='kJ')
            self.add_input('mission:concordia_study:route%s:%s:duration' % (self.options['route'], segment), val=np.nan, units='min')
        self.add_output('mission:concordia_study:route%s:energy' % self.options['route'], units='kJ')
        self.add_output('mission:concordia_study:route%s:energy:propulsion' % self.options['route'], units='kJ')
        self.add_output('mission:concordia_study:route%s:energy:payload' % self.options['route'], units='kJ')
        self.add_output('mission:concordia_study:route%s:energy:avionics' % self.options['route'], units='kJ')
        self.add_output('mission:concordia_study:route%s:duration' % self.options['route'], units='min')
    ...
    def compute(self, inputs, outputs):
        E_mission = sum(inputs['mission:concordia_study:route%s:%s:energy' % (self.options['route'], segment)]
                        for segment in self.options['segments_list'])
        E_propulsion = sum(
            inputs['mission:concordia_study:route%s:%s:energy:propulsion' % (self.options['route'], segment)]
            for segment in self.options['segments_list'])
        E_payload = sum(
            inputs['mission:concordia_study:route%s:%s:energy:payload' % (self.options['route'], segment)]
            for segment in self.options['segments_list'])
        E_avionics = sum(
            inputs['mission:concordia_study:route%s:%s:energy:avionics' % (self.options['route'], segment)]
            for segment in self.options['segments_list'])
        t_mission = sum(inputs['mission:concordia_study:route%s:%s:duration' % (self.options['route'], segment)]
                        for segment in self.options['segments_list'])

        outputs['mission:concordia_study:route%s:energy' % self.options['route']] = E_mission
        outputs['mission:concordia_study:route%s:energy:propulsion' % self.options['route']] = E_propulsion
        outputs['mission:concordia_study:route%s:energy:payload' % self.options['route']] = E_payload
        outputs['mission:concordia_study:route%s:energy:avionics' % self.options['route']] = E_avionics
        outputs['mission:concordia_study:route%s:duration' % self.options['route']] = t_mission
...
class MissionComponent(om.ExplicitComponent):
    ...
    def setup(self):
        for route in self.options['routes_list']:
            self.add_input('mission:concordia_study:route%s:energy' % route, val=np.nan, units='kJ')
            self.add_input('mission:concordia_study:route%s:energy:propulsion' % route, val=np.nan, units='kJ')
            self.add_input('mission:concordia_study:route%s:energy:payload' % route, val=np.nan, units='kJ')
            self.add_input('mission:concordia_study:route%s:energy:avionics' % route, val=np.nan, units='kJ')
            self.add_input('mission:concordia_study:route%s:duration' % route, val=np.nan, units='min')
    ...
    def compute(self, inputs, outputs):
        E_mission = sum(
            inputs['mission:concordia_study:route%s:energy' % route] for route in self.options['routes_list'])
        E_propulsion = sum(
            inputs['mission:concordia_study:route%s:energy:propulsion' % route] for route in self.options['routes_list'])
        E_payload = sum(
            inputs['mission:concordia_study:route%s:energy:payload' % route] for route in self.options['routes_list'])
        E_avionics = sum(
            inputs['mission:concordia_study:route%s:energy:avionics' % route] for route in self.options['routes_list'])
        t_mission = sum(
            inputs['mission:concordia_study:route%s:duration' % route] for route in self.options['routes_list'])
