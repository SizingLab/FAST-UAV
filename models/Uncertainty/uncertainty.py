...
def add_subsystem_with_deviation(group: om.Group, subsys_name: str, subsys: om.ExplicitComponent,
    ...
    for name, unit in uncertain_outputs.items():
        ...
        short_names.append(':'.join(name.split(':')[1:-1]))  # remove first and last ('data:' and ':estimated')
        # else:
    ...
    if uncertain_outputs is not None:
        ...
        subsys_uncertainty.add_variables(long_names=long_names, short_names=short_names, units=units)  # add variables to be modified
        group.add_subsystem(subsys_name + '_deviation', subsys_uncertainty)  # add component to group

        # connect variables
        for i, long_name in enumerate(long_names):
            short_name = short_names[i]
            group.connect(subsys_name + '.' + long_name,
                          subsys_name + '_deviation.uncertainty:' + short_name + ':mean')
            group.promotes(subsys_name + '_deviation',
                           inputs=['uncertainty:' + short_name + ':rel',
    ...
    def add_variables(self, long_names=None, short_names=None, units=None):
        for i, long_name in enumerate(long_names):
            if long_names not in self._long_names:
                ...
                self.add_input('uncertainty:' + short_name + ':rel', val=.0, units=None)
                self.add_input('uncertainty:' + short_name + ':abs', val=.0, units=unit)
...
def add_model_deviation(group: om.Group, subsys_name: str, model, uncertain_parameters: List[str] = None):
    """
    ...
    if uncertain_parameters is not None:
        ...
        subsys_uncertainty.add_variables(names=uncertain_parameters, model=model)  # add variables to be modified
        group.add_subsystem(subsys_name, subsys_uncertainty, promotes=['*'])  # add component to group
...
class ModelDeviation(om.ExplicitComponent):
    ...
    def add_variables(self, names=None, model=None):
        ...
        for i, name in enumerate(names):
            if name not in self._names:
                self.add_input('uncertainty:' + name + ':rel', val=.0, units=None)
                self.add_input('uncertainty:' + name + ':abs', val=.0, units=None)
    ...
    def compute(self, inputs, outputs):
        ...
        if 'uncertain_parameters' in model_attrs.keys(): # and 'init_uncertain_parameters' in model_attrs.keys():
