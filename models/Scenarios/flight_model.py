...
class FlightModel:
    ...
    def get_lift(V, alpha, S_top, C_L0, rho_air):
        ...
        C_L = C_L0 * np.sin(- 2 * alpha)  # [-] flat plate model (minus in front of alpha because of angle definition)
    ...
    def get_AoA(Mtotal, V, theta, S_front, S_top, C_D, C_L0, rho_air):
        ...
        """
        def func(x):
            drag = FlightModel.get_drag(V, x, S_front, S_top, C_D, rho_air)  # [N] drag
            lift = FlightModel.get_lift(V, x, S_top, C_L0, rho_air)  # [N] lift
            weight = Mtotal * g  # [N] weight
            res = np.tan(abs(x - theta)) - (drag * np.cos(theta) + lift * np.sin(theta)) / (weight + drag * np.sin(theta) - lift * np.cos(theta))  # [-] equilibrium residual
            return res
    ...
    def get_thrust(Mtotal, V, theta, S_front, S_top, C_D, C_L0, rho_air):
        ...
        """
        alpha = FlightModel.get_AoA(Mtotal, V, theta, S_front, S_top, C_D, C_L0, rho_air)  # [rad] angle of attack
        weight = Mtotal * g  # [N] weight
        lift = FlightModel.get_lift(V, alpha, S_top, C_L0, rho_air)  # [N] lift
        drag = FlightModel.get_drag(V, alpha, S_front, S_top, C_D, rho_air)  # [N] drag
        thrust = ((weight + drag * np.sin(theta) - lift * np.cos(theta)) ** 2 + (drag * np.cos(theta) + lift * np.sin(theta)) ** 2) ** (1/2)  # [N] thrust requirement
        return thrust, alpha