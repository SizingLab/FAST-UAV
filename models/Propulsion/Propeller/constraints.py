...
class PropellerConstraints(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_output("mission:constraints:speed:forward", units=None)
    ...
    def compute(self, inputs, outputs):
        ...
        prop_con4 = (V_ff - J_forward * W_pro_ff * Dpro / 2 / np.pi) / V_ff

        outputs["data:propeller:constraints:speed:climb"] = prop_con1
        outputs["data:propeller:constraints:speed:forward"] = prop_con2
        outputs["mission:constraints:speed:climb"] = prop_con3
        outputs["mission:constraints:speed:forward"] = prop_con4
    ...
    def compute_partials(self, inputs, partials, discrete_inputs=None):
        ...
        partials[
            ...
            W_pro_ff * Dpro / NDmax**2 / 2 / np.pi
        )
        partials[
            "data:propeller:constraints:speed:forward",
            "data:propeller:geometry:diameter",
        ] = (
            -W_pro_ff / NDmax / 2 / np.pi
        )

        partials["mission:constraints:speed:climb", "specifications:climb_speed",] = (
            J_climb * W_pro_cl * Dpro / V_cl**2 / 2 / np.pi
        )
        partials[
            "mission:constraints:speed:climb",
            "data:propeller:advance_ratio:climb",
        ] = (
            -W_pro_cl * Dpro / V_cl / 2 / np.pi
        )
        partials["mission:constraints:speed:climb", "data:propeller:speed:climb",] = (
