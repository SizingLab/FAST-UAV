...
class ComputeTOW(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        outputs['addons:safety:mission:route%s:tow' % self.options['route']] = tow
...
class ClimbSegment(om.ExplicitComponent):
    ...
    def setup(self):
        # System parameters
        self.add_input('addons:safety:mission:route%s:tow' % self.options['route'], val=np.nan, units='kg')
        ...
        self.add_output('addons:safety:mission:route%s:climb:energy:propulsion' % self.options['route'], units='kJ')
        self.add_output('addons:safety:mission:route%s:climb:energy:payload' % self.options['route'], units='kJ')
        self.add_output('addons:safety:mission:route%s:climb:energy:avionics' % self.options['route'], units='kJ')
        self.add_output('addons:safety:mission:route%s:climb:energy' % self.options['route'], units='kJ')
    ...
    def compute(self, inputs, outputs):
        Mtotal = inputs['addons:safety:mission:route%s:tow' % self.options['route']]
        ...
        C_t = inputs['data:propeller:aerodynamics:CT:axial']
        C_p = inputs['data:propeller:aerodynamics:CP:axial']
        ...
        outputs['addons:safety:mission:route%s:climb:duration' % self.options['route']] = t_cl / 60  # [min]
        outputs['addons:safety:mission:route%s:climb:energy:propulsion' % self.options['route']] = E_cl_pro / 1000  # [kJ]
        outputs['addons:safety:mission:route%s:climb:energy:payload' % self.options['route']] = E_payload / 1000  # [kJ]
        outputs['addons:safety:mission:route%s:climb:energy:avionics' % self.options['route']] = E_avionics / 1000  # [kJ]
        outputs['addons:safety:mission:route%s:climb:energy' % self.options['route']] = (E_cl_pro + E_payload + E_avionics) / 1000  # [kJ]
...
class HoverSegment(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_output('addons:safety:mission:route%s:hover:energy:payload' % self.options['route'], units='kJ')
        self.add_output('addons:safety:mission:route%s:hover:energy:avionics' % self.options['route'], units='kJ')
        self.add_output('addons:safety:mission:route%s:hover:energy' % self.options['route'], units='kJ')
    ...
    def compute(self, inputs, outputs):
        ...
        outputs['addons:safety:mission:route%s:hover:energy:propulsion' % self.options['route']] = E_hover_pro / 1000  # [kJ]
        outputs['addons:safety:mission:route%s:hover:energy:payload' % self.options['route']] = E_payload / 1000  # [kJ]
        outputs['addons:safety:mission:route%s:hover:energy:avionics' % self.options['route']] = E_avionics / 1000  # [kJ]
        outputs['addons:safety:mission:route%s:hover:energy' % self.options['route']] = (E_hover_pro + E_payload + E_avionics) / 1000  # [kJ]
...
class ForwardSegment(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_output('addons:safety:mission:route%s:forward:energy:propulsion' % self.options['route'], units='kJ')
        self.add_output('addons:safety:mission:route%s:forward:energy:payload' % self.options['route'], units='kJ')
        self.add_output('addons:safety:mission:route%s:forward:energy:avionics' % self.options['route'], units='kJ')
        self.add_output('addons:safety:mission:route%s:forward:energy' % self.options['route'], units='kJ')
    ...
    def compute(self, inputs, outputs):
        ...
        D_ff = inputs['addons:safety:mission:route%s:forward:range' % self.options['route']]
        ...
        outputs['addons:safety:mission:route%s:forward:duration' % self.options['route']] = t_ff / 60  # [min]
        outputs['addons:safety:mission:route%s:forward:energy:propulsion' % self.options['route']] = E_ff_pro / 1000  # [kJ]
        outputs['addons:safety:mission:route%s:forward:energy:payload' % self.options['route']] = E_payload / 1000  # [kJ]
        outputs['addons:safety:mission:route%s:forward:energy:avionics' % self.options['route']] = E_avionics / 1000  # [kJ]
        outputs['addons:safety:mission:route%s:forward:energy' % self.options['route']] = (E_ff_pro + E_payload + E_avionics) / 1000  # [kJ]
...
class RouteComponent(om.ExplicitComponent):
    ...
    def setup(self):
        for segment in self.options['segments_list']:
            self.add_input('addons:safety:mission:route%s:%s:energy' % (self.options['route'], segment), val=np.nan, units='kJ')
            self.add_input('addons:safety:mission:route%s:%s:duration' % (self.options['route'], segment), val=np.nan, units='min')
        self.add_output('addons:safety:mission:route%s:energy' % self.options['route'], units='kJ')
        self.add_output('addons:safety:mission:route%s:duration' % self.options['route'], units='min')
    ...
    def compute(self, inputs, outputs):
        E_mission = sum(inputs['addons:safety:mission:route%s:%s:energy' % (self.options['route'], segment)] for segment in self.options['segments_list'])
        t_mission = sum(inputs['addons:safety:mission:route%s:%s:duration' % (self.options['route'], segment)] for segment in self.options['segments_list'])

        outputs['addons:safety:mission:route%s:energy' % self.options['route']] = E_mission
        outputs['addons:safety:mission:route%s:duration' % self.options['route']] = t_mission
...
class MissionComponent(om.ExplicitComponent):
    ...
    def setup(self):
        for route in self.options['routes_list']:
            self.add_input('addons:safety:mission:route%s:energy' % route, val=np.nan, units='kJ')
            self.add_input('addons:safety:mission:route%s:duration' % route, val=np.nan, units='min')
        self.add_output('addons:safety:mission:energy', units='kJ')
        self.add_output('addons:safety:mission:duration', units='min')
    ...
    def compute(self, inputs, outputs):
        E_mission = sum(inputs['addons:safety:mission:route%s:energy' % route] for route in self.options['routes_list'])
        t_mission = sum(inputs['addons:safety:mission:route%s:duration' % route] for route in self.options['routes_list'])

        outputs['addons:safety:mission:energy'] = E_mission
        outputs['addons:safety:mission:duration'] = t_mission
...
class MissionConstraints(om.ExplicitComponent):
    ...
    def setup(self):
        self.add_input('addons:safety:mission:energy', val=np.nan, units='kJ')
        self.add_input('data:battery:energy', val=np.nan, units='kJ')
        self.add_input('data:battery:discharge_limit', val=.8, units=None)
        self.add_output('addons:safety:mission:constraints:energy', units=None)
    ...
    def compute(self, inputs, outputs):
        E_mission = inputs['addons:safety:mission:energy']
        ...
        outputs['addons:safety:mission:constraints:energy'] = energy_con
    ...
    def compute_partials(self, inputs, partials, discrete_inputs=None):
        ...
        partials[
            'addons:safety:mission:constraints:energy',
            'addons:safety:mission:energy',
        ] = -1.0 / (E_bat * C_ratio)
        partials[
            'addons:safety:mission:constraints:energy',
            'data:battery:energy',
