...
class ESCDefinitionParameters(om.Group):
    ...
    """
    def setup(self):
        add_subsystem_with_deviation(self, "power", Power(),
...
class Power(om.ExplicitComponent):
    ...
    def compute_partials(self, inputs, partials, discrete_inputs=None):
        ...
        partials['data:ESC:power:max:estimated',
                 'data:ESC:settings:power:k'] = P_el_to * V_bat / Umot_to

        partials['data:ESC:power:max:estimated',
                 'data:motor:power:takeoff'] = k_ESC * V_bat / Umot_to

        partials['data:ESC:power:max:estimated',
                 'data:battery:voltage'] = k_ESC * P_el_to / Umot_to

        partials['data:ESC:power:max:estimated',
                 'data:motor:voltage:takeoff'] = - k_ESC * P_el_to * V_bat / Umot_to ** 2
...
class Voltage(om.ExplicitComponent):
    ...
    def setup(self):
        self.add_input('data:ESC:reference:power', val=3180., units='W')
