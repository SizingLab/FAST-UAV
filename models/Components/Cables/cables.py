...
@oad.RegisterOpenMDAOSystem("cables")
...
class Radius(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        I_ref = inputs['data:cables:reference:current']  # [A] max allowable current (continuous) of reference cable
...
class Weight(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        mu_ref = inputs['data:cables:reference:density']  # [kg/m] linear mass of reference cable
        I_ref = inputs['data:cables:reference:current']  # [A] max allowable current (continuous) of reference cable
        I = inputs['data:motor:current:hover']  # [A] max current (continuous)
        Larm = inputs['data:structure:arms:length']  # [m] arms length
        Narm = inputs['data:structure:arms:number']  # [-] number of arms (i.e. number of cables)
    ...
    def compute_partials(self, inputs, partials, discrete_inputs=None):
        ...
        partials['data:cables:density', 'data:motor:current:hover'] = mu_ref / I_ref ** (2/1.5) * (2/1.5) * I ** (1/3)
        partials['data:cables:mass', 'data:motor:current:hover'] = mu_ref / I_ref ** (2 / 1.5) * (2 / 1.5) * I ** (
                    1 / 3) * Larm * Narm * 2
