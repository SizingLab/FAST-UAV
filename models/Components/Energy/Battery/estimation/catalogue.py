...
@ValidityDomainChecker(
    {
        'data:battery:voltage:estimated': (DF['Voltage_V'].min(), DF['Voltage_V'].max()),
        'data:battery:capacity:estimated': (DF['Capacity_As'].min(), DF['Capacity_As'].max()),
    },
)
class BatteryCatalogueSelection(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        if self.options["use_catalogue"]:
            ...
            outputs['data:battery:cell:voltage'] = outputs['data:battery:cell:voltage:catalogue'] = V_bat / N_series
