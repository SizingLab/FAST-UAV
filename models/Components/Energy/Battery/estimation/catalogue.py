...
class BatteryCatalogueSelection(om.ExplicitComponent):
    ...
    def setup_partials(self):
        ...
        self.declare_partials(
            "data:battery:energy", "data:battery:energy:estimated", val=1.0, method="fd"
        )
        self.declare_partials(
            "data:battery:current:max",
            "data:battery:current:max:estimated",
            val=1.0,
            method="fd",
        )
        self.declare_partials(
            "data:battery:mass", "data:battery:mass:estimated", val=1.0, method="fd"
        )
        self.declare_partials(
            "data:battery:DoD:max",
            "data:battery:DoD:max:estimated",
            val=1.0,
            method="fd",
        )
    ...
    def compute(self, inputs, outputs):
        ...
        if self.options["use_catalogue"]:
            ...
            # Get closest product
            df_y = self._clf.predict([V_bat_opt, C_bat_opt])
            ...
            E_bat = df_y["Energy_kJ"].iloc[
                0
            ]  # C_bat * V_bat / 1000  # stored energy [kJ]
